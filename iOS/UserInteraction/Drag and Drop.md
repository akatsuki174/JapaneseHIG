...
**ドロップしたテキストに適切なスタイリングを適用してください。** 移動元と移動先で同じテキストスタイル属性がサポートされているなら、ドロップされたテキストは元のフォント、書体、サイズ、その他の属性情報を維持しておくべきです。それ以外の場合は、ドロップされたテキストは移動先のスタイルを採用すべきです。

**ユーザがすぐにドラッグ＆ドロップをすぐに元に戻せない場合は、オプトアウトするための方法をさり気なく、直感的に示してあげることを検討してください。** 例えば共有が発生するアプリでは、ドロップしたコンテンツを投稿する前に中間的な共有シートを表示することができます。この共有シートではステータスメッセージなどの追加コンテンツを提供しつつ、キャンセルボタンも提供することができます。写真が共有フォトストリームにドラッグされると、このような動作表示が現れます。